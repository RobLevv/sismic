...
class TestInterpreterWithDeephistory:
    ...
    def test_deep_memory(self, interpreter):
        ...
        assert set(interpreter.configuration) == {
            "active",
        ...
        assert interpreter.configuration == ["root", "pause"]
        assert set(interpreter._memory["active.H*"]) == {
            "concurrent_processes",
            "process_1",
            "process_2",
            "s12",
    ...
    def test_entered_order(self, interpreter):
        ...
        assert step.entered_states.index("active.H*") < step.entered_states.index(
            "concurrent_processes",
        )
        assert step.entered_states.index("concurrent_processes") < step.entered_states.index(
            "process_1",
        )
        assert step.entered_states.index("concurrent_processes") < step.entered_states.index(
            "process_2",
...
class TestCoverageFromTrace:
    def test_empty_trace(self):
        assert coverage_from_trace([]) == {
            "entered states": Counter(),
            "exited states": Counter(),
            "processed transitions": Counter(),
        }
