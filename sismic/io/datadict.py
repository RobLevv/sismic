    ...
    if stype == "final":
        ...
        if substates and parallel_substates:
            raise StatechartError(
                f'{name} cannot declare both a "states" and a "parallel states" property',
            )
        if substates:
            state = CompoundState(
                name,
        ...
        raise StatechartError(f"Unknown type {stype} for state {name}")
